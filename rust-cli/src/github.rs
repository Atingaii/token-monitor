...
use crate::model::{EncryptedLedger, Ledger, PublicLedger};
...
pub fn ensure_user_fork(token: &str) -> Result<String> {
    ...
    if existing.status().is_success() {
        ...
        if repository.private {
            bail!("{expected} is private. The central zero-server dashboard reads aggregate snapshots from public device branches")
        }
...
impl GithubClient {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        let repository: RepositoryResponse = response.json()?;
        if repository.private {
            bail!("the fork must be public for the zero-server dashboard. Raw sessions are never uploaded; only aggregate public summaries and an encrypted full ledger are written")
        }
        if !repo_is_upstream_fork(&repository) {
    ...
    pub fn replace_snapshot(
        ...
        let encrypted_serialized = serde_json::to_string(envelope)?;
        let public_serialized = serde_json::to_string(&PublicLedger::from(ledger))?;

        let encrypted_blob: ShaResponse = checked(
            self.request(reqwest::Method::POST, format!("{API}/repos/{}/git/blobs", self.repo))
                .json(&json!({ "content": encrypted_serialized, "encoding": "utf-8" }))
                .send()?,
        ...
        let tree: ShaResponse = checked(
            self.request(reqwest::Method::POST, format!("{API}/repos/{}/git/trees", self.repo))
                .json(&json!({ "tree": [
                    { "path": "ledger.json", "mode": "100644", "type": "blob", "sha": encrypted_blob.sha },
