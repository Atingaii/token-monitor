...
pub fn config_dir() -> Result<PathBuf> {
    dirs::config_dir().map(|path| path.join("token-monitor")).context("cannot determine the user configuration directory")
}
...
pub fn normalize_repo(value: &str) -> Result<String> {
    ...
    if parts.len() != 2 || parts.iter().any(|part| part.trim().is_empty()) { bail!("repository must be OWNER/REPO or a github.com repository URL"); }
    Ok(repo)
}

pub fn default_device_name() -> String {
    hostname::get().ok().map(|value| value.to_string_lossy().trim().to_string()).filter(|value| !value.is_empty()).unwrap_or_else(|| format!("{}-device", std::env::consts::OS))
}

pub fn sanitize_device_id(value: &str) -> String {
    let mut out = String::with_capacity(value.len()); let mut last_dash = false;
    for ch in value.trim().chars() {
        if ch.is_ascii_alphanumeric() || matches!(ch, '.' | '_' | '-') { out.push(ch.to_ascii_lowercase()); last_dash=false; }
...
pub fn load()->Result<Config>{
    let path=config_path()?; let data=fs::read(&path).with_context(||format!("configuration not found at {}. Run `token-monitor setup` first",path.display()))?;
    let config:Config=serde_json::from_slice(&data).context("invalid token-monitor configuration")?; if config.version!=2{bail!("unsupported config version {}",config.version);} crate::crypto::decode_key(&config.dashboard_key)?; Ok(config)
}

pub fn save(config:&Config)->Result<()> { let path=config_path()?; if let Some(parent)=path.parent(){fs::create_dir_all(parent)?;} write_private(&path,&serde_json::to_vec_pretty(config)?) }

pub fn read_cached_ledger()->Result<Option<crate::model::Ledger>>{
    let path=ledger_cache_path()?; match fs::read(&path){Ok(data)=>Ok(Some(serde_json::from_slice(&data).context("invalid local ledger cache")?)),Err(error)if error.kind()==std::io::ErrorKind::NotFound=>Ok(None),Err(error)=>Err(error.into())}
}
pub fn write_cached_ledger(ledger:&crate::model::Ledger)->Result<()> { let path=ledger_cache_path()?; if let Some(parent)=path.parent(){fs::create_dir_all(parent)?;} write_private(&path,&serde_json::to_vec(ledger)?) }

fn write_private(path:&Path,data:&[u8])->Result<()> {
    let tmp=path.with_extension(format!("tmp-{}",std::process::id()));
    #[cfg(unix)] { use std::io::Write; use std::os::unix::fs::OpenOptionsExt; let mut file=fs::OpenOptions::new().create(true).truncate(true).write(true).mode(0o600).open(&tmp)?; file.write_all(data)?; file.sync_all()?; }
